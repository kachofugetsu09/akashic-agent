...
| 插件安装、热重载、自验证、Cordis 迁移、plugin-data、Skill、Drift skill、MCP | `projectneed` 第 6、9～13 节 → [0008](decisions/0008-plugin-runtime-publishes-only-committed-snapshots.md) → [0024](decisions/0024-plugin-self-validation-uses-stable-and-latest.md) → [0026](decisions/0026-plugin-rollout-is-owned-by-the-parent-turn.md) → [0036](decisions/0036-plugin-composition-keeps-promotion-owner.md) → [0037](decisions/0037-plugin-services-name-capabilities-not-categories.md) → [0038](decisions/0038-human-commands-are-not-model-tools.md) → [插件 install/uninstall/revert turn 边界发布合同](design/plugin-install-uninstall-turn-boundary-rollout.md) → [插件递归自验证运行时设计](design/recursive-plugin-self-validation.md) → [Cordis 插件迁移能力等价验收](design/cordis-plugin-capability-parity.md) → [插件元能力底座与测试先行合同](design/plugin-meta-capability-task-contract.md) → [插件组合内核第一阶段任务合同](design/plugin-composition-kernel-task-contract.md) → [插件事件与同步执行能力任务合同](design/plugin-event-executor-task-contract.md) → [插件 TopologyView 任务合同](design/plugin-topology-view-task-contract.md) → [插件 lifecycle 接入点任务合同](design/plugin-lifecycle-seam-task-contract.md) → [插件 Turn event leaf import 合同](design/plugin-turn-event-import-task-contract.md) → [Turn committed typed event 合同](design/plugin-turn-committed-event-task-contract.md) → [Observe 与 status_commands 组合迁移合同](design/plugin-observe-status-combination-task-contract.md) → [插件 v3 generation loader 任务合同](design/plugin-v3-loader-task-contract.md) → [插件 assistant 持久 metadata 接入点合同](design/plugin-assistant-metadata-seam-task-contract.md) → [插件 Skill/Dashboard 过渡 Service 合同](design/plugin-assets-service-task-contract.md) → [插件 Skills 组合能力合同](design/plugin-skills-service-task-contract.md) → [插件 UI Slots 组合能力合同](design/plugin-ui-slots-service-task-contract.md) → [插件 Mobile UI generation 绑定合同](design/plugin-mobile-ui-generation-binding-task-contract.md) → [插件 Mobile UI 组合能力合同](design/plugin-mobile-ui-slots-task-contract.md) → [插件 Timer 组合 Service 合同](design/plugin-timer-service-task-contract.md) → [插件 Timer stable snapshot 调度合同](design/plugin-timer-snapshot-scheduling-task-contract.md) → [插件 Tool 组合能力合同](design/plugin-tools-service-task-contract.md) → [插件 Agent Input 组合能力合同](design/plugin-agent-input-service-task-contract.md) → [插件 Commands 组合能力合同](design/plugin-commands-service-task-contract.md) → [插件 Session Read 组合能力合同](design/plugin-session-read-service-task-contract.md) → [持久化状态地图](design/persistence-state-map.md) | `agent/plugin_composition/agent_input.py`、`agent/plugin_composition/commands.py`、`agent/plugin_composition/session_read.py`、`agent/plugin_composition/skills.py`、`agent/plugin_composition/timer.py`、`agent/plugin_composition/tools.py`、`agent/plugin_composition/ui_slots.py`、`agent/plugins/mobile_ui.py`、`agent/plugins/mobile_ui_assets.py`、`agent/turn_events/**`、`agent/tool_hooks/executor.py`、`agent/plugins/install.py`、`agent/plugins/manager.py`、`agent/plugins/snapshot.py`、`agent/plugins/reload_journal.py`、`agent/plugins/turn_rollout.py`、`agent/plugins/skill_links.py`、`agent/plugins/dashboard_host.py`、`agent/control/runtime.py`、`agent/looping/core.py`、`agent/mcp/host.py` |
| 插件 Agent Input、stable callback 跨 reload 准入 | `projectneed` PLG-003、PLG-013～PLG-014 → [插件 Agent Input 组合能力合同](design/plugin-agent-input-service-task-contract.md) → [插件 Timer stable snapshot 调度合同](design/plugin-timer-snapshot-scheduling-task-contract.md) → [插件 Agent Input stable admission 合同](design/plugin-agent-input-stable-admission-task-contract.md) | `agent/plugin_composition/agent_input.py`、`agent/plugins/snapshot.py`、`agent/plugins/manager.py`、`tests/test_plugin_composition_agent_input.py` |
...
│   ├── 0036-plugin-composition-keeps-promotion-owner.md
│   └── 0037-plugin-services-name-capabilities-not-categories.md
├── design/
...
│   ├── plugin-observe-status-combination-task-contract.md
│   ├── plugin-assistant-metadata-seam-task-contract.md
│   ├── plugin-assets-service-task-contract.md
│   ├── plugin-agent-input-service-task-contract.md
│   ├── plugin-agent-input-stable-admission-task-contract.md
│   ├── plugin-meta-capability-task-contract.md
│   ├── plugin-timer-service-task-contract.md
│   ├── plugin-timer-snapshot-scheduling-task-contract.md
│   ├── plugin-tools-service-task-contract.md
