...
| Agent 内骨架、被动链与 Scheduler/Subagent 非特权组合 | `projectneed` 第 6、9～13 节、PLG-018 → [0054](decisions/0054-agent-spine-is-ordinary-plugins.md) → [普通插件 Agent 骨架与被动链迁移合同](design/ordinary-agent-spine-migration.md) → [0034](decisions/0034-turn-is-the-logical-work-unit.md) → [0039（已被取代）](decisions/0039-react-core-atoms-keep-sources-unprivileged.md) → [持久化状态地图](design/persistence-state-map.md) | `agent/looping/core.py`、`agent/core/passive_turn.py`、`agent/control/`、`agent/lifecycle/`、`agent/plugin_composition/`、`agent/plugins/snapshot.py`、`bootstrap/`、`session/`、`plugins/` |
