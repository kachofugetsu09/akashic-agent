...
| Harness benchmark、独立 runtime trial、证据驱动优化 | `projectneed` 第 8～14 节 → [V4 Flash Harness Benchmark 设计](spark/2026-07-30-v4flash-harness-benchmark-design.md) → [Benchmark 诊断循环设计](spark/2026-07-30-agent-benchmark-diagnostic-loop-design.md) → [0010](decisions/0010-provider-default-output-and-benchmark-diagnostics.md) → [逐题结果](benchmark/v4flash-case-results.tsv) → [实验 ledger](benchmark/v4flash-harness-experiment-ledger.md) → [待定语义改变](benchmark/v4flash-semantic-changes-pending.md) → [持久化状态地图](design/persistence-state-map.md) | `benchmark/harbor_v4flash/`、`agent/control/`、`bootstrap/control_execution.py`、`docker/debug/`、独立 artifact store 与 experiment ledger |
