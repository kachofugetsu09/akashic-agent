...
| Akasha 宽 scope 多图分区（proposal） | [宽 scope 分区设计](spark/2026-09-12-akasha-wide-scope-design.md) → [持久化状态地图](design/persistence-state-map.md) → Issue #629 / #370 | `plugins/akasha/`、`core/memory/`、`session/`；proposal 未提升为现行合同前不得直接实现 |
...
| 执行失败、自愈、跳过与重试 | [0063](decisions/0063-execution-failures-have-terminal-results.md) → [持久化状态地图](design/persistence-state-map.md) | `plugins/tools/`、`plugins/delivery/`、`plugins/wake/`、`plugins/akashic_clients/mobile_realtime/` |
