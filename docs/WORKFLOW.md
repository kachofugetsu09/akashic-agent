...
普通 Pull Request 运行 `tests_scenarios/contracts/pr-regression-files.txt` 中的 1080 项 Python 回归、`npm run test:web:pr` 中的 62 项 Web 回归和 change-impact Gate。完整测试资产仍留在仓库；change-impact Gate 可按生产 diff 选择清单外的行为边界，插件候选或发布里程碑还应运行完整 Python/Web 回归和相应的 static fleet、领域组合及 E1～E4 集中 E2E。清理记录和保留理由见[测试与 Gate 清理账本](refactor/test-gate-cleanup-ledger.md)。
