...
普通 Pull Request 运行聚焦后的 Python/Web 回归和 change-impact Gate。插件 v3 的 static fleet、领域组合及 E1～E4 集中 E2E 用于插件候选或发布里程碑，不在每个 Core Pull Request 重复执行。清理记录和保留理由见[测试与 Gate 清理账本](refactor/test-gate-cleanup-ledger.md)。
