...
### ONB-002 Onboarding 持久化每个显式决定并在完成时统一生效

首次 Web onboarding 按“欢迎 → 模型 → 记忆 → 联系方式与主动推送 → 完成”推进。模型是唯一不可跳过的步骤；记忆、联系方式（Telegram、QQBot 与 Android 手机）和主动推送必须同时提供现在配置和“跳过，稍后设置”。`pending`、`configured`、`skipped` 是不同状态：跳过表示用户已经决定本轮不设置，刷新或重新打开后不得重复追问，也不得把跳过伪装成关闭功能或覆盖已有运行配置。完成摘要必须区分已配置、明确关闭和已跳过，并提供开始对话与进入配置中心两个出口。选择连接 Android 手机时可以保存公网 `wss://…/ws` 入口，使手机无需与电脑处于同一网络；频道步骤只保存移动网关配置，不提前重启，完成页在 Gateway 就绪后直接进入扫码与六位确认码认证，用户也可以暂不配对并继续对话。

Onboarding 进度由当前 workspace 的独立状态 owner 原子持久化，浏览器 session、配置 table 是否碰巧存在和运行 readiness 都不得替代该状态。各步骤只保存自己的配置与决定；首次 Gateway 启动集中在完成提交，启动成功后才标记 onboarding 完成。启动失败时停留在完成页并允许重试，重复完成不得重复启动。没有显式状态文件的旧 workspace 可以根据既有模型、记忆和主动配置推导为已完成，但显式状态一旦存在就优先于旧配置推导。

频道与主动推送必须是一条连续链路：未连接的 Telegram、QQBot 或手机不能保存为启用状态的推送目标；界面应把用户带回对应连接，而不是隐藏选项。Telegram chat ID 与 QQBot C2C openid 可以通过用户刚发给 Bot 的消息识别。正在接收主动推送的频道不得直接断开，必须先更换目标或关闭主动推送。模型连接与向量模型在配置中心提供新增、更新和显式移除的完整周期；模型连接移除至少保留一个聊天模型并迁移角色绑定，正在被记忆使用的向量模型必须先切换或关闭记忆。移除均为带 workspace 恢复点的逻辑停用，不物理删除凭据或注册库行；记忆索引迁移不属于本阶段。
