...
Akashic 顶栏、页面导航、当前 route 和浏览器 history。`conversation-ui`、`workbench-ui` 与 `models`
分别注入 `shell.pages.v1`；导航和页面 renderer 是同一个 entry 的两个投影。

`workbench-ui` 声明 `workbench.panels.v1`。Akasha、Wake 以及外置 Dashboard 面板插件通过同一个
...
- 首版移除“知识与运行”顶层 entry，不删除其底层能力或持久数据。
