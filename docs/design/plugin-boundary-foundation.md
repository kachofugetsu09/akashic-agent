...
2026-09-13 阶段顺序调整：维护者要求先确保全部本地插件、默认组合和 Core 底座正确，
第三方外部插件的进一步迁移、CI 与互操作验收后置。已提交的外部改动保留，
本地阶段仍须证明本地插件可以通过正式安装链外置加载、独立组合和替换；
“第三方兼容后置”不免除本地插件的分发与生命周期验收。
...
- `runtime_inspection` 普通插件拥有文档 allowlist 与任务/技能组合。它发布真正的 `RpcMethod`，具体参数模型保留在插件内；Core 的 Web/Mobile adapter 只转换既有客户端协议，不定义新的业务对象接口。
- 每次解析、调用和读取都绑定当前任务的同一 snapshot lease。技能缓存也必须经过实际资产读取口；仅增加引用计数不能代替任务绑定。
- 可选技能/调度缺失时只让相应区域不可用，插件与 MCP 列表仍可查看；provider 异常直接传播。生产路径不为旧同步测试 double 添加兼容分支。
...
工具结果在公开执行返回边界转换为 owner 的结果类型，首调与持久回放不再依赖
消费者或另一个 generation 的类身份。对应归档、拒绝、回复与回执组合 67 项通过。

Message 展示函数依赖消息读取能力，归入公开的 plugin_composition.message_view，
不放宽纯值合同目录的实现依赖检查。真实 Shell 回复测试显式装配 Sources、Models
及相应依赖，保留完成、取消、清理失败和恢复后的原 Output 与进程断言。相关 21 项
通过；渠道、Mobile 与 Message 展示组合 91 项通过。QQ 旧配置迁移在任何写入前
拒绝非有限超时和重复群配置，不在删除旧入口后才暴露非法目标配置。

这些是本地实施证据。整组发行、完整 Gate、CI 和独立概念审查尚待累计栈完成后核对；
不据此声明 refactor 已完成，也没有对正式 workspace 执行迁移。
...
R2 只允许使用清单内既有公开模块；本次登记只是承认已审边界，不扩大运行权限。

| 模块 | 合同 | owner | 为什么是窄缝 |
|---|---|---|---|
| `admission` | `SourceAdmission` / `SOURCE_ADMISSION` | Core 的 snapshot store 与 Root 来源接纳 | 只暴露本 Root 的来源许可与 closed-scope 检查；provider 持有连接生命周期，store 不交给插件 |
