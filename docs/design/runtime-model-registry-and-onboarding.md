...
首次使用按五个阶段推进。模型是唯一必选配置；记忆、联系方式与主动推送都必须让用户在“现在配置”和“跳过，稍后设置”之间做出明确选择，不把跳过误写成运行配置：
...
`<workspace>/onboarding-state.json` 是这条流程唯一的进度 owner，记录当前步骤、完成状态以及 memory/channel 各自的 `pending | configured | skipped`。`skipped` 是用户已完成的一种决定：刷新和重开浏览器后不再追问，但不创建、关闭或覆盖对应运行配置；配置中心仍可随时修改真实设置。没有显式状态文件的旧 workspace 仅在模型、memory 和 proactive 配置都已存在时推导为已完成，避免升级后强迫重走。

模型、记忆与联系方式步骤在 onboarding 中只提交配置和进度，不逐步重启 Gateway。Android 手机选项在联系方式步骤写入移动网关配置；可选公网 `wss://…/ws` 入口归 `mobile_realtime.public_url` 所有，存在时配对与通信不要求手机和电脑处于同一局域网。完成页先通过 Supervisor bridge 成功启动或重载，再把 `completed` 提交为真，并在移动配对管理端点就绪后显示二维码与六位确认码。用户暂不配对只结束本次配对交互，不回滚已完成的 onboarding 或已启用的移动网关。启动失败保持在完成页并显示错误。重复完成是幂等操作，不重复启动。普通配置中心保存仍按各自设置接口立即生效。

| 状态 | 页面重点 | 可见操作 | 失败语义 |
|---|---|---|---|
| 无模型连接 | 一组连接方式 | 选择 Codex、OpenCode 或 API 服务 | 不连接 Chat WebSocket，不显示原始网络错误 |
| 登录或验证中 | 当前连接表单 | 完成授权、检测模型、保存 | 错误留在当前模态框，可修正后重试 |
| 已有模型、未决定记忆 | 记忆方式 | Akasha、经典记忆、关闭记忆、跳过并稍后设置 | 关闭会保存正式配置；跳过只保存决定状态 |
...
| 未决定联系方式与主动推送 | 可渐进展开的 Telegram、QQBot、Android 手机与主动推送设置 | 保存选择，或跳过并稍后设置 | 跳过不改动已有 channel/proactive/mobile 配置；未连接频道不能成为启用状态的目标 |
| 所有决定已记录、尚未完成 | 完成摘要 | 开始对话、连接手机，或查看配置中心 | Gateway 启动失败时不提交 completed；移动管理端点未就绪时不打开无效二维码，可原地重试 |
...
配置中心延续同一依赖链：Telegram 与 QQBot 可以连接、更新、从新消息识别投递目标并显式断开；手机可以更新局域网或公网入口。主动推送目标始终显示全部支持项，但未就绪项提供返回联系方式的动作。正在作为启用状态目标的频道拒绝断开。聊天模型连接和向量模型均提供新增、更新、带恢复点的逻辑停用；聊天连接停用会把角色迁到仍可用的连接且至少保留一个聊天模型，活动向量模型则要求先切换或关闭记忆。已有记忆数据的索引迁移保持 change lock，本阶段不实现迁移。
