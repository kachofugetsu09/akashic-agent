...
- **I（普通插件 provider）：** 已安装 `akashic_clients` artifact 的 `mobile_webui/release_cli.py` 拥有 canonical manifest、发布仓、ticket 和 HTTP 数据面，以及 build/import/promote/rollback/GC 命令；它接收 `--source-repository` 作为 frontend 输入，但实现始终来自当前 artifact，不由 checkout 的 `plugins` 包提供。`plugins/akashic_clients/mobile_realtime/` 只读已提交的 `ReleaseView`；`schema_cli.py` 与相关移动端测试共同构成 provider 的机器可读证据。旧 `infra/mobile_*` 与 `scripts/publish-mobile-webui.py` 路径已经退役。
