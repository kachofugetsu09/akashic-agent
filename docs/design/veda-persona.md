...
3. 首次配置时 Prompt setup 只可创建缺失文件；运行期只有 Main Agent 在用户明确要求修改人格或 Veda 时可以写入。后台 optimizer、Proactive、Wake 和 Drift 只读。
4. 本轮 prompt 在模型执行前冻结；本轮对 Veda 的写入只从下一次 prompt 组装开始生效。
5. 合法 Veda 必须存在、为非空 UTF-8。正常 Prompt 读取遇到缺失或损坏时失败，不使用代码 fallback，也不自动修复。
6. 仓库保存版本化默认 Markdown。正式安装后的通用 `main.py setup` 只在缺失时调用 Prompt 包的 setup 创建；已有合法内容不覆盖。候选验证使用隔离 workspace，不写正式 workspace。
7. `python /path/to/installed/prompt/persona.py --workspace PATH` 是 runtime 加载前的显式恢复入口。它先保存原始字节和 hash，再原子发布当前版本默认内容。

## 状态边界

- Veda 是必须纳入 workspace 备份和恢复核对的权威 Markdown。
- Prompt setup 与代码升级不得覆盖用户定制人格；恢复默认必须使用名称明确的命令。
