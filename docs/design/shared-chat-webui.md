...
renderer 可提供 `prefetch(context)`，只执行一次轻量读取，不创建卡片 DOM。已提交过程在邻近可视区域预取首个思考卡片；未展开的预取不轮询，主动展开可以提升尚在网页队列中的同一读取优先级。进行中的可见卡片仍按已有间隔刷新，失败保留已显示内容并提供局部重试。页面重载会丢弃内存结果，缓存不是新的召回权威状态。
