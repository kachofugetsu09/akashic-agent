...
| `plugin-data/computer/state/` | Computer Workload 中的 Chromium 增加并原位更新 profile；Gateway 更新活动状态；OpenCLI 更新登录刷新状态 | 容器停止、插件更新、禁用或普通卸载只让数据暂时无 writer，不改变或删除登录资料；重启只清除 Chromium 自己的 `Singleton*` 临时锁并收束未完成 activity | 只有名称明确的 Computer 数据永久删除操作可以物理减少；必须先停唯一 Workload writer、备份整个目录并核对恢复方式 |
...
| `runtime/plugin-reloads.sqlite3` + `runtime/root-switch/artifacts/` + `runtime/root-switch/configs/` | 每次热重载增加 transaction 与阶段事件；首次启动 candidate/formal runtime 前固定 old/new snapshot、old/new generation、base/candidate artifact pointer 与 `runtime_owner_boot_id`。RootSwitch 在第一项共享 owner 动作前增加 closed plan，并为每个 part 保存两边 Core 推导的 direct `SwitchInputRef` multiset、stable registration Fiber、dependency closure、exact ref、`use_new=0`、immutable code 与 private exact config；首次公开 part 也必须在第一次 `recover` 前增加 absence → exact transition 并 pin，`recover` 成功后才原子选择新边，禁止另写无事务 stable choice。service hold reserve 另增加全局 HoldId、holder identity + ServiceKey namespace 与 versioned `RootRecipe`：普通插件 holder 来自该唯一 part 的 stable registration Fiber，snapshot 外 durable host 来自构造时 sealed identity；recipe 固定 Core ABI、source lineage、exact plugin/config/topology、provider Fiber、SwitchPart inputs/needs 与 runnable sealed registry identity，并立即 pin 全部 artifact；不保存 payload、secret、live binding 或 socket | reload transaction、RootSwitch step 与 choice 只按既有单调状态机推进。service hold 只允许 `reserved → active → dropped`；live source 在 RootScope 内 reserve/activate 后退出 scope，再用 transaction permit 从 unbound task call；boot 使用窄 permit。恢复以 fresh retired/closed Root 和新 process-local runtime ID 保留原 source lineage；recipe/ABI/provider/registry 不符只能进入 degraded，call=0。transition 清理后 choice 继续作为 selected ref 或 absence tombstone；path、hash、mode 漂移保持 degraded | reload、choice、hold 与 recipe 都是恢复/审计证据，没有普通 retention。RootSwitch transition 只在 selected snapshot 与完整 ref 已证明后标 cleared；pin 只有不再被 selected choice、pending plan 或 reserved/active hold 引用时才能物理减少。source owner durable 证明 done/abort 后才可 drop hold；unknown 不 drop；数据库 row 当前没有普通自动删除协议 |
| `runtime/plugin-jobs/outcomes.sqlite` | 普通 `jobs` 插件的唯一 current scanner 在 live exact lease 中先 reserve HoldId，再 INSERT semantic job/event/interval identity、exact plugin/model/artifact/source/handler identity 与 `queued` row，最后 activate hold；registry 同时冻结 contributor module | activate 后 scanner 退出 current RootScope，再从 unbound task 用 transaction permit call exact held recipe。同一 invocation 只按 `queued → running → terminal/retry_pending/unknown` 前向更新；`running` 必须早于 handler/provider/programmatic Turn effect。QUEUED crash 可 exact replay；RUNNING crash 不自动 replay，写或报告 unknown/degraded并保留 hold。held old Root 只执行固定 service，永不启动 scanner | terminal row 必须先 drop hold，再由当前 scanner delete；reserved 无 row 可由同 holder 证明后 drop。unknown/held row 当前不得减少；普通插件卸载、重载或日志清理不得删除。workspace 备份以 SQLite online backup + integrity_check 保存；只有后续名称明确的 retention/插件数据管理操作可减少 |
