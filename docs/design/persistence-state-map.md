...
| release `activation/attempt-external-*.json`、`failed-*.json`、`settled-failed-*.json`、`active.json` | 外部 plan 纯预检后、任何实际升级前保存 pending attempt；失败保留 phase/恢复点，成功在 Core/Bridge 与完整选中 Fiber 核对后写 active | pending、失败和选中 Root 是不同事实；仅相同 plan/image/有序输入的成功 active receipt 加 doctor/live 证明只读重放。启动前失败只有在实际整份 state/env 恢复、旧 Root/active 与 reload owner 无未决事实均核对后才另写 settlement | 不自动删除 pending 或失败记录。未知提交和启动中断留在 maintenance；启动后效果没有对应 owner 证据时不得用 settlement 命令放行，单个指针或人工成功声明不足以恢复 |
