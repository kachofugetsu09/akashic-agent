...
| `runtime/plugin-reloads.sqlite3` + `runtime/root-switch/artifacts/` + `runtime/root-switch/configs/` | 每次热重载增加 transaction 与阶段事件；首次启动 candidate/formal runtime 前固定 old/new snapshot、old/new generation、base/candidate artifact pointer 与 `runtime_owner_boot_id`；RootSwitch 在第一项共享 owner 动作前增加 closed plan，并为每个 part name 保存 old/new exact ref、`use_new=0` 与两边 immutable code、private exact config file 和 dependency closure；config value 与 secret 不复制进 journal，config root/subdir 为 `0700`、file 为 `0600`；首次公开 part 在 admission 前增加 stable choice | 同一 reload transaction 按状态机更新 phase、failure resource、recovery target/action、单调 attempt 和 retry receipt；RootSwitch step 只单调前进，全部成功后一次把 plan 与每个 `switch_choices` choice 改为 `use_new=1`；transition 清理后 `switch_choices` 继续作为 selected ref 或 absence tombstone，下一次 boot 不回退到可变代码或 config；恢复经正常 config parser 读取 journal 所选 private file，path、hash、mode 漂移保持 degraded；failure 只允许 `cleanup_failed → degraded` 单调强化且 target 不可覆盖，同 boot 不做进程清理，新 supervised boot 只按已记录 old boot ID 恢复 | reload 事故记录当前没有自动 retention；恢复和审计仍依赖的记录不得自动删除。RootSwitch transition 只在 selected snapshot 与完整 ref 已证明后标 cleared；code/config pin 只有不再被 selected choice 或任一 pending plan 引用时才能物理减少；数据库 row 当前没有普通 retention |
