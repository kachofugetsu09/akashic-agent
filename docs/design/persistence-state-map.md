...
- `plugin-data/wake-builtin/wake.sqlite3` 只保存 admission、seen Content、decision run 与 Timer attempt。每次真实 Timer fire 先追加 attempt，再读取 EventMail watermark；水位读取前可以是 `NULL`。`no_due`、`content_insufficient`、`admission_rejected`、`shared`、`model_skip`、`deferred`、`cancelled_after_fire`、`delivery_unknown`、`failed` 都分别闭合并可由 Dashboard 查询。`cancelled_after_fire` 只表示 Timer 已触发、runtime 在职责检查前关闭，不产生 scoped Turn 或外部发送。新进程在领域恢复前把上一进程遗留的 `checking` 保守闭合为 `delivery_unknown`，因为崩溃前可能已经越过 provider I/O。attempt 是运行诊断，不拥有三类 mail 正文。
