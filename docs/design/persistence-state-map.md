...
| `sessions.db/inbound_handoffs` | Mobile 消息在进入内存处理前 INSERT 完整 handoff 与 `session + client_message_id` 去重身份 | pending 期间由 MessageBus 持有 durable handoff custody；ChannelGenerationHost 以 exact binding 调用 `CHANNEL_INPUT`，canonical user 已存在时只对账、不重开回复；Reply 与 Delivery 的 Output 终态独立提交；完整进程退出则保留 durable row，由下一 boot 按有限页恢复 | Input、handoff cleanup 或后续 durable delivery 任一提交失败都不得减少原 handoff；DELETE 的暂态 `OSError` 保留 owner 并进入 `cleanup_degraded` 重试 | ChannelGenerationHost + MessageBus custody；handoff row、canonical Input、Output receipt、recovery report |
