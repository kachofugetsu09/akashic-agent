...
| 候选新链 `owner_records` 的 compaction `summary:*` 与 `head:*` | compaction 创建只读摘要记录，包含连续 Prompt 覆盖、parent/generation、正文与模型调用出处；version 2 另存本代真实摘要输入和明确省略消息，摘要与 head 同事务提交 | 只有该 owner 可按 parent CAS 推进 head；摘要只允许完全相同的幂等重放，不覆盖旧 generation。省略消息只退出 Prompt，不冒充摘要或 Markdown 学习输入 | 无自动减少协议；旧 ledger/prepare/receipt 保留。恢复读取原生 SQLite 备份；发布失败整笔回滚。第 08 层候选已接入生成、原始归档读取与 Output 使用引用；旧摘要由 `20260908_01_legacy_summaries` 只增转换当前有效祖先链；version 0 保存完整原行与 digest，旧原生 version 1 只读兼容，新记录写 version 2。失效旧分支和 prepare/receipt 保留；非空 prepare 拒绝转换。迁移回执 `migration:legacy-summary-v1` 与记录/head 同事务增加且不自动减少；删除失效与正式 workspace 切换仍待验收 |
...
- Markdown plugin 在 `markdown-profile-writes.db` 中按文档保存 before-image、draft 和 applied receipt，并在启动时前向恢复。
  ...
  学习请求按完整 Turn 的切点分批；历史迁入消息只展示原文正文与已校验的出处角色，不重复投送原始 provider extra 和工具回放。原 Message 保持不变；旧 version 0/1 沿原累计来源学习，version 2 只学习各代 `summary_message_ids`，明确省略消息不取得学习资格。模型只返回新增条目的文档、章节、单行正文和实际消息 ID；Markdown owner 保留既有档案，从同一条目生成完整 v2 草稿与证据表，不让模型重复抄写整份档案或改写旧事实。每批以当前内存草稿继续，全部批次及来源证据通过后才交给原 writer；失败或取消不写中间档案、不推进 receipt。单个完整 Turn 可以超过 32k token 的批次目标，但不能超过模型窗口预算；无法容纳时明确失败，不裁切原文。新增条目和模型推理共用输出预算，沿模型声明的生成上限；能力未知时交给 provider 默认值。模型草稿不满足格式或证据合同时，同一批原文与 before-image 只修正一次；连续不合格交回 follower 延时重试。持久草稿损坏和原文解析错误仍明确失败，不转换成模型重试。
