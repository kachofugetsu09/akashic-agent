...
| `plugin-data/computer/state/` | Computer Workload 中的 Chromium 增加并原位更新 profile；Gateway 更新活动状态；OpenCLI 更新登录刷新状态；Computer MCP 以原子替换增加 `screenshots/` 文件 | 容器停止、插件更新、禁用或普通卸载只让数据暂时无 writer，不改变或删除登录资料；重启只清除 Chromium 自己的 `Singleton*` 临时锁并收束未完成 activity；普通卸载保留有界截图集合 | 只有名称明确的 Computer 数据永久删除操作可以减少 profile、活动或登录资料，且必须先停唯一 Workload writer、备份整个目录并核对恢复方式。Computer MCP 是截图 retention 的唯一删除 owner：每次成功写入后只保留最新 32 张，并删除遗留临时文件 |
