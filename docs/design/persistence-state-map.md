...
| `sessions.db/tool_result_artifacts` | 主 Agent 被动 session turn 的成功纯文本工具结果达到 8192 字符时，在首次 provider 消费前 INSERT 完整 immutable 正文；同一 session/call ID 只允许完全相同的幂等重试 | 不允许原位更新或逻辑失效；prompt placeholder 是临时投影，不改 artifact、messages 或 tool_chain | 普通运行没有 TTL/GC；只有用户显式 session cascade 删除可在同一已验证 SessionDB 备份后 DELETE |
| `sessions.db/tool_result_reads` | `read_tool_result` 成功返回一个同 session 有界切片时，在同一写事务 INSERT offset、limit、returned chars 与 reader turn evidence | 不允许原位更新或逻辑失效；失败、越权和越界读取不追加记录 | 只随用户显式 session cascade 删除，并由同一个删除前 SessionDB 备份恢复 |
