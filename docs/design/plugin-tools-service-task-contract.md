...
- 本 PR 仍临时读取 v2 `MetadataKind.TOOL`，仅用于迁移期混合 catalog。对应 v2 删除目标是 `@tool` metadata 收集、`_build_plugin_tool`、`PluginManager._register_tools` 与旧 plugin Tool 注册路径；最后一个 v2 Tool consumer 迁移并通过等价 Gate 后物理删除，不保留 deprecated adapter。
