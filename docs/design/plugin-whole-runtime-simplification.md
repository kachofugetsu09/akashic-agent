...
调用 `use_source()`；新 models/driver 读取现有设置，凭据刷新沿原 connection owner，
调用账仍写本次独立环境。不复制模型库，不借用旧 Root 的模型执行对象，不新增底座接口。
缺失或损坏设置仍明确失败。可信插件合同见 PLG-001 和 0071 勘误，实际运行尚未验证。
