...
- Scheduler/Subagent 的已确认边界见 [React Core 原子能力与 Scheduler/Subagent 插件设计](design/react-core-scheduler-subagent.md) 和[分阶段任务合同](design/react-core-scheduler-subagent-task-contract.md)：先建立真实场景差分 runner，再收口来源无关的 Core 原子能力，随后分别迁移两个仓库内置非特权插件。Proactive/Wake 只做结构验算，当前不迁移、不改状态；每个实施切片仍需独立 worktree、收窄合同和批准差异。
