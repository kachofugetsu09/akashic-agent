...
- Scheduler/Subagent 的已确认边界见 [React Core 原子能力与 Scheduler/Subagent 插件设计](design/react-core-scheduler-subagent.md) 和[分阶段任务合同](design/react-core-scheduler-subagent-task-contract.md)：S0～S3 已完成；`spawn`/`spawn_manage` 现由仓库内置非特权 v3 插件提供，child 递归走同一 scoped Turn；Core one-shot Timer、完整逻辑消息 Delivery 与声明式 workspace file 已成为来源无关 Service，Scheduler v3 shadow 通过真实 loader 组合 fixed wait、stateless SOFT Turn、delivery 与 settlement，但没有读取正式 schedule 或启动 timer，旧 Scheduler 仍是唯一生产 owner。下一步 S4 运行完整差分 fixture 后切换 binding、删除旧 runtime/Tool adapter。Proactive/Wake 只做结构验算，当前不迁移、不改状态。
