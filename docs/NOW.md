...
- Scheduler/Subagent 的已确认边界见 [React Core 原子能力与 Scheduler/Subagent 插件设计](design/react-core-scheduler-subagent.md) 和[分阶段任务合同](design/react-core-scheduler-subagent-task-contract.md)：S0 已建立确定性 runner、scoped Turn port/handle、Tool grant、typed receipt 与 one-shot Timer 合同并删除失败的 `tool_loop_guard:` 控制暗号；S1 已让仓库内置非特权 v3 Subagent 插件通过公开 `SCOPED_TURNS` Service 在 shadow fixture 中递归取得同一 Turn terminal，candidate 零 child admission，且没有切换正式 `spawn` owner。下一步 S2 补齐同步/后台/profile/容量/终态/取消/重载差分 fixture 与 mutants，全部等价后才切 binding 并删除独立推理循环。Proactive/Wake 只做结构验算，当前不迁移、不改状态；每个后续切片仍需独立 worktree、收窄合同和批准差异。
