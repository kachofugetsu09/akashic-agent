...
- Scheduler/Subagent 的已确认边界见 [React Core 原子能力与 Scheduler/Subagent 插件设计](design/react-core-scheduler-subagent.md) 和[分阶段任务合同](design/react-core-scheduler-subagent-task-contract.md)：S0 基建、S1 Subagent shadow 和 S2 正式 owner 切换已完成；`spawn`/`spawn_manage` 现由仓库内置非特权 v3 插件提供，child 递归走同一 scoped Turn，profile 通过 exact grant 与 Turn-local Tool 实例保持任务目录/网络限制，四类 mutant 有独立 oracle，旧 `SubAgent` 独立循环已删除。下一步 S3 实现来源无关 one-shot Timer 并建立 Scheduler v3 shadow。Proactive/Wake 只做结构验算，当前不迁移、不改状态。
