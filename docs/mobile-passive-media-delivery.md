...
- 交互：命令项从“填入输入框后再次确认”改为 Telegram 式动作语义；点按后立即发送一次并收起，输入草稿不被覆盖。退出动画期间的连续点击由同步门闩拦截。
- 附件边界：新增独立 `sendCommand` 链路，命令 payload 明确不读取 `AttachmentDraftStore`，因此 `media_refs=[]`，不会把已选附件夹带到 `/undo`，也不会消费附件草稿。
- 面板：移除全屏 `ModalBottomSheet`、标题区和遮罩，改为贴着输入栏的单一 Material 3 transient surface；高度在 `240–304dp` 间受窗口约束，目录只在内部滚动。说明位于左侧、命令位于右侧，当前菜单按钮以主色圆形和关闭图标表达展开状态。
...
| Before | After |
| --- | --- |
| Modal sheet 遮罩改变整页明度，命令入口没有明确展开状态 | 面板复用不透明 `surfaceContainerHighest`；展开按钮只用既有 `primary/onPrimary` 状态色，不新增色系 |

better-typography：

| Before | After |
| --- | --- |
| `titleLarge` 标题和说明占据有限视窗，命令列使用固定 144dp 左栏 | 移除重复标题；`bodyMedium` 说明在左、等宽命令在右，保留 56dp 行高和两行截断 |

better-ui：

| Before | After |
| --- | --- |
| 命令填入输入框后仍需手动发送 | 点命令立即执行一次并收起，草稿保持原样 |
| 440dp 全屏遮罩式 BottomSheet 隐藏输入栏关系 | 240–304dp 输入栏附着面板，无全屏遮罩，输入栏持续可见 |
| 普通发送链路会顺带消费附件草稿 | 独立纯文本命令链路，附件保持在 composer 中等待用户发送 |
