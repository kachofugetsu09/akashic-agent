...
def _stable_copy(source: Path, destination: Path) -> None:
    ...
    raise RuntimeError(f"普通文件在快照期间持续变化: {source}")
...
def _copy_workspace(
    ...
    # 1. 先固定路径清单；遍历不跟随任何软链。
    paths = sorted(source.rglob("*"), key=lambda item: item.relative_to(source).parts)
    excluded_prefixes: list[Path] = []
    for item in paths:
        relative = item.relative_to(source)
        if any(_is_relative_to(relative, prefix) for prefix in excluded_prefixes):
            continue
        reason = _excluded_reason(relative, is_symlink=item.is_symlink())
...
def prepare_rehearsal(
    *,
    ...
    try:
        ...
        records, exclusions, databases = _copy_workspace(
