...
class AgentTick:
    ...
    def _build_system_prompt(self, ctx: AgentTickContext, gw: GatewayResult) -> str:
        ...
        return (
            ...
            "【你的任务】\n"
            "1. 对本轮原材料逐条判断：这条内容是否可能让用户不感兴趣，是否可能不符合规则，是否值得进入 interesting。\n"
            "2. 你的主工作是分类，不是主动研究新题材，不是主动扩展候选池。\n"
            "3. 你要基于规则和用户偏好，把本轮原材料分成 interesting 和 not_interesting。\n\n"
            "【你的输出】\n"
            "1. 对每条候选给出最终分类：mark_interesting 或 mark_not_interesting。\n"
            "2. 如果最终没有 interesting，调用 skip(no_content)。\n"
            "3. 如果最终有 interesting，生成一条最终消息并 send_message。\n\n"
            "【工具职责】\n"
            "1. Workspace 主动上下文：这是用户当前明确提出并要求你遵守的规则集合。它定义你该怎么筛、哪些要先验证、哪些必须过滤；它不提供新闻事实。\n"
            "2. recall_memory：判断单条内容是否可能是用户雷点，或是否可能让用户感兴趣。\n"
            ...
            "Content 的主要任务不是做研究，而是把本轮候选逐条分成 interesting 或 not_interesting。\n"
