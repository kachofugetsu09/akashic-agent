...
`load_artifacts` 与 `render_content` 把已提交内容投影为模型可读块：只读取入参
（附件经只读 `AttachmentReadPort`）并返回冻结值，不写会话、不调用模型。
Core 与多个插件共用它们，因此由合同层拥有，实现留在 `plugins/models/content.py`。
"""
...
from agent.plugin_contracts.message_read import MessageCatalogPort
from agent.plugin_contracts.message_codec import json_value


async def load_artifacts(
    reader: AttachmentReadPort,
...
async def read_content(
    message: Message,
    catalog: MessageCatalogPort,
    artifacts: AttachmentReadPort,
) -> tuple[str | File, ...]:
    ...
    refs = {
        ref.artifact_id: ref
        for ref in catalog.reader(message.session_id).attachments(message.message_id)
    }
    parts: list[str | File] = []
