...
内容 schema、引用区间、终态副作用判定与 `ContentView` 读写接口是插件与 Core
共用的结构边界；它们只解释消息内容，不持有存储或生命周期。实现留在
`plugins/content/{api,plugin}.py`，由本模块拥有定义。

`Content`（具体读写实现）留在插件，合同层只用 `ContentView` Protocol 描述它。
"""
...
CONTENT = ServiceKey[ContentView]("content.v1")
