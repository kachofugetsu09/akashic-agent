...
let request: ApiRequest = (path, init) => fetch(path, init);

/** Bind this bundled plugin copy to the host-owned, generation-aware HTTP port. */
export function bindApiRequest(next: ApiRequest): () => void {
