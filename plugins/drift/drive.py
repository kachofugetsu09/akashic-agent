...
def advance_drift_drive(
    *,
    ...
    return DriftDriveResult(
        ...
        recent_drift_suppression,
        repetition_score,
        _reasons(content, recent_drift_suppression, repetition_score, attempt),
    )
...
def _integrated_idle_drive(idle_hours: float, scale: float) -> float:
    return scale * (idle_hours - 4.0 * (1.0 - math.exp(-idle_hours / 4.0)))


def _reasons(
    content: float, recent_drift: float, repetition: float, attempt: bool
) -> tuple[str, ...]:
    reasons: list[str] = []
    if content >= 0.5:
