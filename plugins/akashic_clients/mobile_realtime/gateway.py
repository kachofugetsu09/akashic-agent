...
from typing import Any, TYPE_CHECKING, NoReturn, cast
...
from ..config import MobileRealtimeConfig
from .auth import (
...
from .inbox import DurableInboxManager, InboxResetRequired
from agent.plugin_composition.message_view import follow_messages
from .message_view import bounded_reply_status, message_chunks
from ..services import MessageReaderPort as MessageReader
from ..services import turn_milestone
...
from ..mobile_webui.http import (
    VerifiedWebUiTicket,
    WebUiTicketError,
    WebUiTicketIssuer,
    parse_single_range,
)
from ..mobile_webui.manifest import ManifestError, canonical_manifest_bytes
...
from ..mobile_webui.store import (
    MobileWebUiStore,
...
class MobileGatewayRuntime:
    ...
    async def _resume_and_register(
        ...
        *,
        ...
        #    客户端 active_turns 只是可重建投影，不能驱动服务端伪造终态。
        async with self._delivery_lock:
            if self._stopping:
                return
...
def build_mobile_gateway_runtime(
    ...
    *,
    ...
    webui_source_repository: Path | None = None,
    ...
    try:
        ...
        from ..mobile_webui.auto_publish import auto_publish_webui

        if webui_source_repository is not None:
            auto_publish_webui(
                webui_source_repository,
                workspace,
                server_id=keyset.manifest.server_id,
            )
