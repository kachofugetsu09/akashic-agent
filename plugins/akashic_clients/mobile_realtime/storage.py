...
_TRANSPORT_CONTINUITY_RESET_REASON = "transport_continuity_lost"
...
class MobileRealtimeStorage:
    ...
    def read_transport_boot_id(self) -> str | None:
        """读取最近一次成功声明的宿主 boot identity。"""

        with self._lock:
            row = self._db.execute(
                """
                SELECT last_transport_boot_id
                FROM mobile_transport_boot
                WHERE singleton = 1
                """
            ).fetchone()
        if row is None:
            return None
        value = row["last_transport_boot_id"]
        if value is not None and not isinstance(value, str):
            raise TypeError("mobile_transport_boot.last_transport_boot_id 必须为文本或 NULL")
        return value

    def mark_transport_boot(
        self,
        boot_id: str,
        *,
        created_at: datetime,
    ) -> tuple[DurableInboxEvent, ...]:
        """声明宿主 boot，并为旧 transport continuity 追加一次 reset 事件。

        marker、每台活动设备的 inbox 事件和 cursor 在同一 SQLite 事务中提交。
        既有未 ACK 事件与命令收据不会被清理或重排。
        """

        boot_key = _require_text(boot_id, "boot_id")
        ...
        with self._lock, self._db:
            ...
            previous = None if marker is None else marker["last_transport_boot_id"]
            if previous is not None and not isinstance(previous, str):
                raise TypeError(
                    "mobile_transport_boot.last_transport_boot_id 必须为文本或 NULL"
                )
            if previous == boot_key:
                return ()

            # 首次安装只记录宿主身份，不为尚不存在的 transport continuity 伪造 reset。
            if previous is None:
                _ = self._db.execute(
                    """
                    INSERT INTO mobile_transport_boot(singleton, last_transport_boot_id)
                    VALUES(1, ?)
                    ON CONFLICT(singleton) DO UPDATE SET
                        last_transport_boot_id = excluded.last_transport_boot_id
                    """,
                    (boot_key,),
                )
                return ()
            ...
            for row in rows:
                ...
                _ = self._db.execute(
                    ...
                    """,
                    (device_id, event_seq, event_id, envelope, timestamp),
                )
                updated = self._db.execute(
                    ...
                    """,
                    (event_seq + 1, device_id, event_seq),
                )
                if updated.rowcount != 1:
                    raise RuntimeError(f"设备 event_seq 分配发生并发冲突: {device_id}")
                events.append(
                    DurableInboxEvent(
                        device_id=device_id,
                        event_seq=event_seq,
    ...
    def _init_schema(self) -> None:
        ...
        _ = self._db.executescript(
            ...
            CREATE TABLE IF NOT EXISTS mobile_transport_boot (
                singleton INTEGER PRIMARY KEY CHECK(singleton = 1),
                last_transport_boot_id TEXT
            );
...
def _encode_stored_event(
    *,
    ...
    """编码不含连接态字段的 durable event envelope。"""

    return json.dumps(
        {
            "id": _require_text(event_id, "event_id"),
            "type": _require_text(event_type, "event_type"),
            "payload": payload,
        },
        ensure_ascii=False,
        separators=(",", ":"),
...
def _new_ulid() -> str:
    """生成 Mobile durable event 使用的协议合法 ULID。"""
