...
class MessageMemory:
    ...
    async def prepare(self, snapshot: tuple[Message, ...], source: str) -> Materials:
        """Context 的实际输入前缀产生一次查询；记录成功发布后才交付引用。"""
        async with self._lock:
            self._check_open()
            if not snapshot:
                return Materials("")
            ...
            if self._catalog.reader(session_id).snapshot(through_seq=snapshot[-1].seq) != snapshot:
            ...
            async with self._bindings.open(self._learning_binding, AKASHA_LEARNING) as (learning, metadata):
                ...
                vectors = self._embeddings.bind(learning.text)
                ...
                if missing:
                    values = await self._embed_batch([learning.text(message) for message in missing])
                ...
                recall = await run_memory_job(lambda: query_memory(
                    self._consumer.cycle, self._consumer.state, learning_binding=self._learning_binding,
                    text=text, dense=dense, stamp=stamp, source=origin, limit=self._limit,
                ))
                identity = uuid4().hex
                material = render_materials(identity, recall, learning, self._catalog, max_chars=self._max_chars)
                ...
                references.update((ref.ref, ref) for ref in tool_references(
                    snapshot, source, learning, self._bindings, self._records,
                ))
