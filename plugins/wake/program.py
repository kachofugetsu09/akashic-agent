...
from plugins.react.plugin import StepLimit
...
async def run(ctx: Context, task: Task, reader: MessageReader, request: Request) -> Message:
    ...
    try:
        return await ctx.require(REPLY_EXECUTE)(
                         ...
                         max_output_tokens=0,
                         max_steps=3 if phase.stage == 'screen' else 1 if phase.stage == 'alert' else 20 if phase.stage == 'investigate' else 40,
