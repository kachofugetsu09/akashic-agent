...
async def test_turn_committed_recent_context_consumer_serializes_same_session():
    ...
    async def _refresh_recent_turns(request) -> None:
    ...
    register_turn_committed_consumers(
        ...
        memory_engine=cast(Any, consolidation),
    )
...
@pytest.mark.asyncio
async def test_turn_committed_consumers_schedule_consolidation_without_post_memory():
    event_bus = EventBus()
    session = MagicMock()
    session.key = "cli:1"
    session_manager = SimpleNamespace(get_or_create=MagicMock(return_value=session))
    ...
    engine.refresh_recent_turns = AsyncMock()
    register_turn_committed_consumers(
        event_bus=event_bus,
        session_manager=cast(Any, session_manager),
        scheduler=cast(Any, scheduler),
