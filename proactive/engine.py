...
from proactive.energy import composite_score, d_content, d_energy, d_recent, random_weight
...
class ProactiveEngine:
    ...
    async def tick(self) -> float | None:
        ...
        if self._cfg.anyaction_enabled and self._anyaction:
            ...
            if not should_act:
                logger.info("[proactive] gate_result=reject selected_action=null meta=%s", meta)
        ...
        new_items, new_entries, semantic_duplicate_entries = self._sense.filter_new_items(items)
        logger.info(
        ...
        if self._cfg.feature_scoring_enabled:
            ...
            should_send = bool(decision_message.strip()) and (
                feature_final_score is not None and feature_final_score >= self._cfg.feature_send_threshold
            )
            logger.info(
...
def _feature_final_score(
    *,
    ...
    risk = (
        ...
        + cfg.feature_weight_interrupt_penalty * (1.0 - max(0.0, min(1.0, interruptibility)))
    )
    system_bonus = (
